...
%% new price demand data
price_scen_data = load('C:\Code\Matlab\Risk-aversion\data\price_scen.mat');
demand_scen_data = load('C:\Code\Matlab\Risk-aversion\data\dem_scen_redu.mat');

price_actual_data = load('C:\Code\Matlab\multi-regional energy trading\robust_multi_periode\actual_price.mat');
demand_actual_data = load('C:\Code\Matlab\multi-regional energy trading\robust_multi_periode\actual_load.mat');
%%%%%


sample_actual_price = price_actual_data.market_data_2016(48*2+1:48*3,6) + 10;
sample_actual_demand = demand_actual_data.demand_data_2016(48*2+1:48*3,6) + (4800-4634);


%%%%
price_scenarios = price_scen_data.price_scen;
price_probability = price_scen_data.pscen_prob;

demand_scenarios = demand_scen_data.dem_scen;
demand_probability = demand_scen_data.scen_red_prob;

sample_demand = demand_scenarios(48*2+1:48*3,6:end-5);
sample_price = price_scenarios(48*2+1:48*3,6:end-5);

sample_demand_max = maxk(sample_demand,5,2);
sample_demand_max = sample_demand_max(:,5);

sample_demand_min = mink(sample_demand,5,2);
sample_demand_min = sample_demand_min(:,5);

sample_demand_mean = median(sample_demand,2);


sample_price_max = maxk(sample_price,5,2);
sample_price_max = sample_price_max(:,5);

sample_price_min = mink(sample_price,5,2);
sample_price_min = sample_price_min(:,5);

sample_price_mean = median(sample_price,2);

% figure();
% plot(sample_demand_max);hold on;
% plot(sample_demand_min);hold on;
% plot(sample_demand_mean); hold off;
% 
% figure();
% plot(sample_price_max);hold on;
% plot(sample_price_min);hold on;
% plot(sample_price_mean); hold off;
...
%%
    ...
    [hhh, phh] = fanChart(1:48, price_scenarios(48*2+1:48*3,6:end-5),'median',5:5:95); hold on;
    %eee = plot(sample_actual_price,'c--','LineWidth',2);hold on;
    
    aaa = plot(sample_price_max,'b--');hold on;
    bbb = plot(sample_price_min,'r--');hold off;
    %plot(sample_price_mean); hold off;
 
    
    xlabel('market period') 
    ylabel('spot [SGD/MWh]')  
    %legend([phh(8),hhh,aaa,bbb,eee],'95\% confidence interval','mean value of all scenarios','upper bound','lower bound','actual data')
    legend([phh(8),hhh,aaa,bbb],'95\% confidence interval','mean value of all scenarios','upper bound','lower bound')
    ...
    subplot(2,1,2);
    fanChart(1:48, demand_scenarios(48*2+1:48*3,6:end-5),'median');hold on;
    %plot(sample_actual_demand,'c--','LineWidth',2);hold on;
    plot(sample_demand_max,'b--');hold on;
    plot(sample_demand_min,'r--');hold off;
    %plot(sample_demand_mean); hold off;
    
    
    xlabel('market period') 
    ylabel('demand [MW]') 
    %legend('95\% confidence interval','mean value of all scenarios')
    %legend('Location','SouthEast','Orientation','vertical');
    %legend show;
    grid on;
...
%     filename='forecast_load_vs_price';
%     export_fig(gcf,filename,'-nocrop','-pdf','-r300','-painters');

%%


number_of_market_period = 48;

psp_price = sample_price_mean(1:number_of_market_period);
load_profile = sample_demand_mean(1:number_of_market_period)/max(sample_demand_mean(1:number_of_market_period));

max_deviation_from_psp_price =  sample_price_max(1:number_of_market_period) - psp_price;

max_deviation_from_load_est =  sample_demand_max(1:number_of_market_period) - sample_demand_mean(1:number_of_market_period);
max_deviation_from_load_est_normalized = max_deviation_from_load_est./(max(sample_demand_mean(1:number_of_market_period)));
...
level_of_conserv = 8;
    ...
    % robust load
        ...
        max_deviation_load = max_deviation_from_load_est_normalized;
    ...
    for t = 1:time_step
        ...
        psp_cost = psp_cost + QG_var(1,t)*psp_price(t)/3;
    end
...
ADMM.var.rho = 1e2*ones(mpc.cluster.number,1);%8e-2 for t = 24 0.05
...
%             SOC_max = 0.99;
            ...
            %% robust optimization varibales %price
            mu_sum = sdpvar(1,1);
            mu_prox = sdpvar(1,time_step);
            % robust constraint 
            robust_constr = [];
            
            % robust load variables 
            mu_sum_D = sdpvar(1,1);
            mu_prox_D = sdpvar(1,time_step);
...
%                 end
           ...
           %% % line current constr
           if  c == 3
               v_all_bus_var = [ones(1,time_step)*mpc_cluster_array(c).gen(1,VG); VM_var]; % 5% maximal error is tolerated
               %VM_var
               ...
               for tttt = 1:time_step
                    congestion_constr = [congestion_constr, -0.05*sqrt(2)<= line_current_magnitude(1,tttt)<=0.05*sqrt(2)];
               end 
           end  
           ...
           %%
            ...
            % robustification constraint 1 @price
            robust_constr = [mu_sum>=0];

            % robustification constraint 2 @price 
            for t = 1:time_step
                robust_constr = [robust_constr, mu_sum + mu_prox(t) >= max_deviation_from_psp_price(t)*(PG_var(1,t)), mu_prox(t)>= 0]; 
            end
            
            % robust constraint for the load
            robust_constr_D = [mu_sum_D>=0];

            for t = 1:time_step
                robust_constr_D = [robust_constr_D, mu_sum_D + mu_prox_D(t) >= max_deviation_load(t)*P0, mu_prox_D(t)>= 0]; 
            end
            
            
            if c == 1 
                constr = [PQ0_constr,V_constr,PQ_constr,Batt_constr,SOC_constr,robust_constr,robust_constr_D];
            elseif c == 2 
                constr = [PQ0_constr,V_constr,PQ_constr,Batt_constr,SOC_constr,robust_constr_D];
            else 
                constr = [PQ0_constr,V_constr,PQ_constr,Batt_constr,SOC_constr,robust_constr_D,congestion_constr];
            end
        ...
        elseif c == 2   % cluster 2 & 5 yes
                ...
                for t = 1:time_step
                    ...
                    dg_cost = dg_cost + (mpc_cluster_array(c).gencost(2,5)'*PG_var(mpc_cluster_array(c).gen(2:end,1),t) + PG_var(mpc_cluster_array(c).gen(2:end,1),t)'*diag(mpc_cluster_array(c).gencost(2,6))'*PG_var(mpc_cluster_array(c).gen(2:end,1),t) + mpc_cluster_array(c).gencost(4,5)'*QG_var(mpc_cluster_array(c).gen(2:end,1),t));
...
plot(0:number_of_market_period,[soc_init(1) soc_res(1,:)],'b*','DisplayName','SOC BESS 1 - Central'); hold on;
plot(0:number_of_market_period,[soc_init(2) soc_res(2,:)],'c*','DisplayName','SOC BESS 2 - Central'); hold on;
plot(0:number_of_market_period,[soc_init(3) soc_res(3,:)],'r*','DisplayName','SOC BESS 3 - Central'); hold on;


plot(0:number_of_market_period,[soc_init(1) ADMM.var.SOC{1}],'bo','DisplayName','SOC BESS 1 - ADMM'); hold on;
plot(0:number_of_market_period,[soc_init(2) ADMM.var.SOC{2}],'co','DisplayName','SOC BESS 2 - ADMM'); hold on;
plot(0:number_of_market_period,[soc_init(3) ADMM.var.SOC{3}],'ro','DisplayName','SOC BESS 3 - ADMM'); hold on;
grid on;
...
bar(psp_price,'g','DisplayName','nodal price at PCC'); hold on;
plot(temp,'r-*','DisplayName','MG 1: exchange price','LineWidth',2); hold on;
plot(temp2,'b-','DisplayName','MG 2: exchange price','LineWidth',2); hold on;
plot(temp3,'y:','DisplayName','MG 3: exchange price','LineWidth',2); hold on;
ylim([30 70]);
xlabel('market period');
ylabel('price [S$/MW]','FontSize',fntsize,'FontName','Times New Roman');
grid on;
legend('Location','North','Orientation','vertical');
legend('boxon')
legend show;
...
ylim([30 70]);
xlabel('market period');
ylabel('price [S$/MWh]','FontSize',fntsize,'FontName','Times New Roman');
grid on;
legend('Location','North','Orientation','vertical');
legend('boxon')
legend show;
...
subplot(2,1,2)
plot(p_inout(1,:),'r:*','DisplayName','MG 1 - bus 7','LineWidth',1); hold on;
plot(p_inout(2,:),'g:*','DisplayName','MG 2 - bus 7','LineWidth',1); hold on;
plot(p_inout(3,:),'k:*','DisplayName','MG 3 - bus 7','LineWidth',1); hold on;
title('b)')
x = [0, number_of_market_period+2, number_of_market_period+2, 0];
y = [0, 0, 4, 4];
c = [0 0.5 0];
...
filename='congested_case';
export_fig(gcf,filename,'-nocrop','-png','-r300','-painters');
