...
for t = 1:time_step
% p_0, q_0 constr
    PQ0_constr = [PQ0_constr, PG_var(1,t) == -sum(PG_var(2:end,t)), QG_var(1,t) == -sum(QG_var(2:end,t))];
    V_constr = [V_constr, 0.90*ones(nbu-1,1)/1.001 <= deltaVA(1:(nbu-1),t) + vbase <= 1.05*ones(nbu-1,1)*1.00]; % 5% maximal error is tolerated
    PQ_constr = [PQ_constr, PG_min(2:end)<=PG_var(2:end,t)<=PG_max(2:end),QG_min(2:end)<=QG_var(2:end,t)<=QG_max(2:end), PG_min(1)<= PG_var(1,t)+P0*load_profile(t) <= PG_max(1), QG_min(1)<= QG_var(1,t)+Q0*load_profile(t)  <= QG_max(1)];
...
for t = 1:time_step
    robust_constr = [robust_constr, mu_sum + mu_prox(t) >= max_deviation_from_psp_price(t)*(PG_var(1,t)+P0*load_profile(t)), mu_prox(t)>= 0]; 
end
...
for t = 1:time_step
    ...
    psp_cost = psp_cost + (PG_var(1,t)+P0*load_profile(t))*psp_price(t) + (QG_var(1,t)+Q0*load_profile(t))*psp_price(t)/3;
end

% robustification objective 
for t = 1:time_step
    robust_cost = robust_cost + (mu_prox(t) + (PG_var(1,t)+P0*load_profile(t))*psp_price(t));
end
...
cost_debug_central = zeros(1,4);
cost_debug_central(1) = value(battery_cost);
cost_debug_central(2) = value(psp_cost);
cost_debug_central(3) = value(dg_cost);

v_mag_central_res = value(deltaVA(1:(nbu-1),2) + vbase);

energy_import_psp_res = Pvar_res(1,:)'+P0*load_profile;


%% derive the robust prices - worst case scenario 
PCC_schedule = Pvar_res(1,:)' + P0*load_profile;
...
%             cost = battery_cost + psp_cost/7 + dg_cost; % 7 to scale down the price to normal price
        ...
        elseif c == 2   % cluster 2 & 5 yes
                ...
                for t = 1:time_step
                    ...
                    ADMM.var.Lambda{c,t}(1)*(PG_var(1,t)+P0*load_profile(t) - ADMM.var.p_global{t}(c,1)) + ...% 
                    0.5*ADMM.var.rho(c)*(PG_var(1,t)+P0*load_profile(t) - ADMM.var.p_global{t}(c,1))^2 + ...% 
                    ADMM.var.Lambda{c,t}(2)*(QG_var(1,t)+Q0*load_profile(t) - ADMM.var.q_global{t}(c,1)) + ...% 
                    0.5*ADMM.var.rho(c)*(QG_var(1,t)+Q0*load_profile(t) - ADMM.var.q_global{t}(c,1))^2;
        ...
        elseif c == 3 %  cluster 3 yes 
            ...
            for t = 1:time_step  
                ...
                coupled_cost = coupled_cost + ...
                ADMM.var.Lambda{c,t}(1,1)*(PG_var(1,t)+P0*load_profile(t) - ADMM.var.p_global{t}(c,1)) + ...% 
                0.5*ADMM.var.rho(c)*(PG_var(1,t)+P0*load_profile(t) - ADMM.var.p_global{t}(c,1))^2 + ...%
                ADMM.var.Lambda{c,t}(1,2)*(QG_var(1,t)+Q0*load_profile(t) - ADMM.var.q_global{t}(c,1)) + ...%
                0.5*ADMM.var.rho(c)*(QG_var(1,t)+Q0*load_profile(t) - ADMM.var.q_global{t}(c,1))^2;
...
%         end
        ...
        for t = 1:time_step
            ...
            elseif c== 2 
                ...
                ADMM.var.p_global_temp{t}(c,mpc.CoupIdx{c}) = PGres(1,t) + P0*load_profile(t); % from new PF point % p.u 
                ADMM.var.q_global_temp{t}(c,mpc.CoupIdx{c}) = QGres(1,t) + Q0*load_profile(t);

%                ADMM.var.v_global_temp(c,temp) = v0_res; 
            elseif c == 3
                %coupled bus 1
                ADMM.var.p_global_temp{t}(c,1) = PGres(1,t) + P0*load_profile(t); 
                ADMM.var.q_global_temp{t}(c,1) = QGres(1,t) + Q0*load_profile(t);    
            end
        end
